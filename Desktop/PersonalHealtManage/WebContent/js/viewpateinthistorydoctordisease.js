...
Ext.onReady(function() {
	...
	patNameStore=Ext.create('Ext.data.Store', {
	    ...
	    proxy: {
	        type: 'ajax',
			url:contextPath+'/dia/patnamesdisease.do',
	        reader: {
	            type: 'json',
	            root:'model'
	                }
	...
	testIdStore=Ext.create('Ext.data.Store', {
	    ...
	    proxy: {
	        type: 'ajax',
			url:contextPath+'/dia/testnamesdisease.do',
	        reader: {
	            type: 'json'
	                }
	    },
	...
	questionStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
			...
			url : contextPath + '/dia/viewanalysisForPatNameAndTestIdDisease.do',
			reader : {
				type : 'json',
				root : 'model'
			}
	...
	function obtainTestNamesForPatName(patName)
	{
		...
		store.load(
			{
				url :contextPath+'/dia/viewspecifictestForPatNameDisease.do',
				params:{
						patName:patName
						}
			}
	...
	function sendTestTable(testIdSelected)
	{
		...
		store.load(
			{
				url :contextPath+'/dia/viewspecifictestdisease.do',
				params:{
						testId:testIdSelected
						}
			}
